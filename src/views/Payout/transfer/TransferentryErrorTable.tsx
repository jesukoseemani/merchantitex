...
const TransferentryErrorTable = () => {

    const dispatch = useDispatch();
    const history = useHistory();

    const [unverified, setUnverified] = useState<TransHistory[]>([]);
    ...
    const getCustomers = async () => {
        ...
        try {
            ...
            // console.log(res.data.find((x) => x.status === "unverified"))

            let newData = history?.filter((x) => x?.status === "unverified")
            setUnverified(newData)
    ...
    return (
        <>
            ...
            <Box mt="17px" className={styles.tableContainer}>
                <CustomClickTable
                    columns={columns}
                    rows={rows}
                    totalRows={totalRows}
...
export default TransferentryErrorTable
